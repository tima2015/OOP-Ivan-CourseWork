...
public:
    ...
    Nanok(const char *fileName = nullptr);
...
private:
    ...
    /**
     * Флаг
     * true сли запущен режим выделения текста.
     */
    bool isSelect = false;

    /**
     * Координаты с которых было начато выделение
     */
    int bix = -1, biy = -1;

    /**
     * Начало/Завершение выделение текста
     */
    void select();

    /**
     * Копирует текущее выделение в буффер обмена
     */
    void copy();

    /**
       * Вставляет текст из буффера обмена в текст
       */
    void paste();

    /**
       * Открывет инструкцию по использованию программы. э
       * После нажатия любой клавиши возвращается в редактор.
       */
    void openHelp();
};
